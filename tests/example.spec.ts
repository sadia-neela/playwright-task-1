...
test('Create user , add products to cart, verify cart, delete user', async ({ customBrowser: browser, request, baseURL }) => {
  const userData = user;
  const productID1 = '1';
  const productID2 = '2';
  const apiClient = new UserApiClient(request, baseURL!);
  try {
    //go to home page
